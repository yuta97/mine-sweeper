...
export const Main = () => {
	...
	return (
        <>
            <h1>Mine Sweeper!</h1>
            <div>
                {initMap.map((v,i) => <Line key={i} line={v} open={open} setOpen={setOpen} flag={flag} setFlag={setFlag} y={i} />)}
            </div>
        </>
	);
}
...
const Line = ({line, open, setOpen, y, flag, setFlag}) => {
    return (
        <div style={{display: "flex"}}>
            {line.map((v,i) => <Block  key={i} element={v} open={open} setOpen={setOpen} flag={flag} setFlag={setFlag} x={i} y={y} />)}
        </div>
	);
}